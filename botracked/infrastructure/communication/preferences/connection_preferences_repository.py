...
class ConnectionPreferencesRepository:
    '''
        Defines class ConnectionPreferencesRepository with attribute(s) and method(s).
        Repository managing file persistence for connection preferences using ats_utilities.

        It defines:

            :attributes:
                | DEFAULT_CONFIG_FILE - Default configuration file path in user home.
                | _config_file - Resolved path to active preferences file.
                | _context - ATS ContextBundle instance for file operations.
            :methods:
                | __init__ - Initializes repository with optional storage path.
                | load_preference - Reads previously saved port and baud rate.
                | save_preference - Persists active port and baud rate.
                | __str__ - Returns string representation of repository.
    '''

    DEFAULT_CONFIG_FILE: ClassVar[Path] = (
        Path.home() / '.config' / 'botracked' / 'connection.json'
    )
    _config_file: Path
    ...
    def save_preference(self, port: str, baud: int = 9600) -> bool:
        '''
            Persists active port and baud rate to storage using ATS Storer.

            :param port: Serial port identifier (e.g. '/dev/ttyUSB0').
